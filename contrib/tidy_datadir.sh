...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Unitecoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Unitecoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Unitecoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Unitecoin 0.7 datadir."
    ;;
  3)
    echo "Detected Unitecoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Unitecoin 1.0 datadir."
    ;;
esac
