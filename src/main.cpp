...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Unitecoin: starting difficulty is 1 / 2^12
...
{
    // Unitecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 50 * COIN; //TEMPLATE

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 3 years //TEMPLATE
    nSubsidy >>= (nHeight / 840000); // Unitecoin: 840k blocks in ~3 years //TEMPLATE

    return nSubsidy + nFees;
}

static const int64 nTargetTimespanOld = 60 * 60 * 24 * 2; // Unitecoin: 2 days //TEMPLATE
static const int64 nTargetTimespan = 60 * 60 * 2; // Unitecoin: 2 hours //TEMPLATE
static const int64 nTargetSpacing = 2 * 60; // Unitecoin: 2 minutes //TEMPLATE
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Starting from block 44,800 we changed the difficculty to 16
    // new retarget interval has been set to 60
...
//        printf("GetNextWorkRequired RETARGET (fork at block 44800)\n");
...
//        return bnNew.GetCompact();
    ...
    // Unitecoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    if (pindexLast->nHeight > COINFIX1_BLOCK) {
        blockstogoback = nReTargetHistoryFact * nIntervalLocal;
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Unitecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Unitecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfc, 0xd9, 0xb7, 0xdd }; // Unitecoin: increase each by adding 2 to bitcoin's value.  //TEMPLATE
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UnitecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnitecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UnitecoinMiner(CWallet *pwallet)
{
    printf("UnitecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("unitecoin-miner");
    ...
    try { loop {
        ...
        printf("Running UnitecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UnitecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UnitecoinMiner, pwallet));
}
